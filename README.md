...
- **Boot splash** — short Codex-style draw-in animation when `clade`
  starts (any key skips; `--no-splash` or `CLADE_NO_SPLASH=1` to
  disable; automatically skipped when stdout isn't a TTY).
