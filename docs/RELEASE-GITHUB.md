...
PATH="$HOME/.bun/bin:$HOME/.local/bin:$PATH" \
  bash scripts/build.sh --version=1.2.1 --with-code --with-graphify
```
...
```bash
git tag -a 1.2.1 -m "PrAImate 1.2.1"
git push origin main
git push origin 1.2.1

gh release create 1.2.1 \
  --repo sPROFFEs/praimate \
  --target main \
  --title "PrAImate 1.2.1" \
  --notes-file RELEASE_NOTES.md

gh release upload 1.2.1 --clobber \
...
`1.2.1` build as current and that the GitHub release is neither draft nor
prerelease.
