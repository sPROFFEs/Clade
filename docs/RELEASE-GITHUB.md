...
PATH="$HOME/.bun/bin:$HOME/.local/bin:$PATH" \
  bash scripts/build.sh --version=1.2.0 --with-code --with-graphify
```
...
```bash
git tag -a 1.2.0 -m "PrAImate 1.2.0"
...
gh release create 1.2.0 \
  ...
  --target main \
  --title "PrAImate 1.2.0" \
  --notes-file RELEASE_NOTES.md

gh release upload 1.2.0 --clobber \
...
`1.2.0` build as current and that the GitHub release is neither draft nor
prerelease.
