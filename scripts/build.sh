...
build_one() {
  ...
  GOOS="$goos" GOARCH="$goarch" CGO_ENABLED=0 \
    go build -trimpath -ldflags "$LDFLAGS" -o "$out/waifu$ext" ./cmd/waifu
    ...
    case "$goos" in
      windows)
        if command -v zip >/dev/null 2>&1; then
          ( cd dist && zip -qr "waifu-$VERSION-$triplet.zip" "$triplet" )
        elif command -v tar >/dev/null 2>&1; then
          ( cd dist && tar -czf "waifu-$VERSION-$triplet.tar.gz" "$triplet" )
